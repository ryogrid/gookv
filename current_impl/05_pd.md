...
| `--max-peer-count` | `3` | Maximum number of replicas per region |
...
> **Important: `--max-peer-count` must match the number of initial voters.**
> gookv bootstraps the initial region with ALL nodes listed in `--initial-cluster` as Raft voters. If `MaxPeerCount` is less than the number of initial voters, the PD scheduler will immediately begin removing peers via ConfChange to reduce the replica count. This causes continuous ConfChange churn (leadership instability, `confVer` incrementing, stale client caches) that can prevent the cluster from stabilizing. Always set `--max-peer-count` to match the number of KVS nodes when all nodes are initial voters.
...
- **PD server binary** (`cmd/gookv-pd`) with `--addr`, `--data-dir`, `--cluster-id`, `--max-peer-count`, `--pd-id`, `--initial-cluster`, `--peer-port`, `--client-cluster`, `--log-level`, `--log-file` flags.

...
gookvs includes a Placement Driver (PD) client package (`pkg/pdclient`) for interacting with an external PD server. PD is the cluster-level metadata and coordination service responsible for:
...
The PD client speaks the kvproto `pdpb.PD` gRPC service, which means gookvs can connect to a real TiKV PD server without modification.
...
The `proto/pdpb.proto` file defines the `PD` gRPC service with the following RPCs (subset relevant to gookvs):

| RPC | Request/Response Style | Used by gookvs |
|-----|----------------------|----------------|
...
## 5. PD in gookvs Architecture

```mermaid
sequenceDiagram
    participant App as Application / TiDB
    participant GS as gookvs-server
...
- **PD is external to gookvs.** gookvs does not embed a PD server; it relies on a separately deployed PD (the same PD used by TiKV/TiDB).
- **`--pd-endpoints` flag** is accepted by `cmd/gookvs-server/main.go` and stored in `config.PDConfig.Endpoints`. The default is `127.0.0.1:2379`.

...
- **GC safe point RPCs are implemented.** PDServer handles `GetGCSafePoint` and `UpdateGCSafePoint`. However, these RPCs are not yet exposed in the `pdclient.Client` interface -- the GC worker currently uses in-process safe point tracking rather than calling PD.
...
- `PutRegion` / `GetRegionByID` / `GetRegionByKey` -- CRUD for region metadata. `GetRegionByKey` uses linear scan over all regions (sufficient for the current scale).
- `UpdateStoreStats` -- records the latest `StoreStats` from a heartbeat.
...
| `RegionHeartbeat` | Bidi stream | Loops receiving heartbeat requests, updates region metadata and leader in MetadataStore, sends empty response (no scheduling commands yet). |
...
- `grpcClient` -- complete gRPC implementation for all 13 methods, including bidirectional streaming for `Tso` and `RegionHeartbeat`. `ReportRegionHeartbeat` uses `CloseSend()` + `Recv()` for reliable delivery.
...
- **TSO integration.** Timestamps arrive in client requests as pre-allocated `uint64` values. The server does not call `GetTS` itself; that responsibility belongs to the upstream SQL layer (TiDB). The `Tso` RPC is fully functional on both client and server, but no component within gookv-server calls it automatically.
...
- **Split coordination via PD.** `AskBatchSplit` and `ReportBatchSplit` are implemented at both the client and server level. The split checker exists in the raftstore, but the PD-coordinated split proposal path (where PD tells a peer to split via `RegionHeartbeatResponse.split_region`) is not wired in the peer event loop. Currently splits are reported after the fact but not initiated by PD.
- **GC safe point client methods.** `GetGCSafePoint` and `UpdateGCSafePoint` are implemented on PDServer but not yet exposed in the `pdclient.Client` interface. The GC worker uses in-process safe point tracking.
- **Scheduling commands.** `RegionHeartbeat` on PDServer always returns an empty response. No scheduling logic (leader transfer, peer change, merge) is implemented server-side.
